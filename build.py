...
LOG_LEVEL_VAL = 1  # Default to INFO (1)
...
def validate_kernel_match_logic(rel_path):
    if rel_path.suffix not in [".hpp", ".cu"]:
        return
...
def compile_project(targets=None):
    ...
    cxx_flags = [f"-I{ROOT_DIR}", f"-DTG_LOG_LEVEL={LOG_LEVEL_VAL}"]
    nvcc_flags = [
        f"-I{ROOT_DIR}",
        "-std=c++20",
        "-x",
        "cu",
        f"-DTG_LOG_LEVEL={LOG_LEVEL_VAL}",
    ]
...
def main():
    ...
    global USE_CUDA, DEBUG_MODE, NO_LINT, PROFILE_MODE, DISABLE_OPENCL, LOG_LEVEL_VAL
    USE_CUDA = args.cuda
    ...
    LOG_LEVEL_VAL = log_level_map.get(log_level_str, 1)

    console.print(
        f"\n[bold cyan]Starting One-Click Build [{'DEBUG' if DEBUG_MODE else 'RELEASE'}] (Log Level: {log_level_str})...[/bold cyan]\n"
    )
    lint()
