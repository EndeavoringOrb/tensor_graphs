...
/**
 * FUSED KERNEL: ADD FP32 3D + 0D (In-place Broadcasting)
 * Pattern: Input3D[b, s, d] += Input0D[]
 */

inline bool matchAddFP32_3D_0D_Inplace(const std::vector<TensorNode> &inputs, const TensorNode &output)
{
    if (inputs.size() != 2)
        return false;
...
inline void runAddFP32_3D_0D_Inplace(const std::vector<const void *> &inputs, const std::vector<void *> &outputs,
...
inline uint32_t refFactoryAdd3D_0D_Inplace(const std::vector<uint32_t> &inputs, Graph &graph)
{
    if (inputs.size() != 2)
...
REGISTER_FUSED_KERNEL_INPLACE("Add_3D_0D", 2, Backend::CPU, matchAddFP32_3D_0D_Inplace, runAddFP32_3D_0D_Inplace, refFactoryAdd3D_0D_Inplace);