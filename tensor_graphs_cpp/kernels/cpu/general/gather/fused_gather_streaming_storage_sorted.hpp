...
// matrix into memory. The basic streaming gather avoids the full load but
// executes arbitrary random disk reads and may redundantly load identical
// tokens.
//
...
// 5. Direct-stores the computed FP32 vectors back to their correct sequence
// positions.
...
// Match function
// ---------------------------------------------------------------------------
inline bool matchGatherStreamingStorageSorted(const std::vector<TensorNode> &inputs, const TensorNode &output)
{
...
// ---------------------------------------------------------------------------
static inline bool gather_sorted_readFromFileAtOffset(int fd, uint64_t offset, void *buf, uint64_t bytes)
{
    if (bytes == 0)
        return true;
...
inline void runGatherStreamingStorageSorted(const KernelContext &ctx)
{
    ...
    if (fd < 0)
    {
        Error::throw_err("Gather_StreamingStorage_Sorted_NEON: expected STORAGE "
                         "input for W (fd[0] >= 0).");
    }
    ...
    struct IndexTracker
    {
        ...
        // Sort primarily by token ID to group identical tokens and linearize disk
        // offsets
...
// Reference Factory
// ---------------------------------------------------------------------------
inline LogicalId refFactoryGatherStreamingStorageSorted(const std::vector<LogicalId> &inputs, Graph &graph)
{
