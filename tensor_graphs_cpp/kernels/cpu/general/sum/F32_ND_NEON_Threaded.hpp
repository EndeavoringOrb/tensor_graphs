...
inline void runSumF32_ND_Threaded(const std::vector<const void *> &inputs, const std::vector<void *> &outputs,
    ...
    for (uint32_t t = 0; t < num_threads; ++t)
    {
        workers.emplace_back([=]()
            ...
            for (uint32_t o = o_start; o < o_end; ++o) {
                // Path A: Reduction on the very last dimension (Contiguous horizontal sum)
                if (inner == 1) {
