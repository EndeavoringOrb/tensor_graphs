...
inline bool matchRMSNormBF16_3D_Inplace_Threaded(const std::vector<TensorNode> &inputs, const TensorNode &output)
{
...
inline void runRMSNormBF16_3D_Inplace_Threaded(const std::vector<const void *> &inputs, const std::vector<void *> &outputs,
        ...
        workers.emplace_back([=]() {
            ...
            for (uint32_t row = start_row; row < end_row; ++row)
            {
                ...
                for (uint32_t d = 0; d < D; ++d)
                {
                    ...
                    out_row[d] = out_row[d] * inv_std * (w_f32 + 1.0f);
                }
            }
        });
    }
...
inline uint32_t ref_rms_bf16_broadcast_scalar_inplace_threaded(Graph &g, uint32_t scalar_id, uint32_t B, uint32_t S, uint32_t D, bool full_d = false)
{
...
inline uint32_t refFactoryRMSNormBF16_inplace_Threaded(const std::vector<uint32_t> &inputs, Graph &graph)
{
    ...
    uint32_t n_node = ref_rms_bf16_broadcast_scalar_inplace_threaded(graph, graph.constant({1}, &d_float, DType::FLOAT32), B, S, 1, false);
    uint32_t mean_sq = graph.div(sum_sq, n_node);

    float eps = 1e-6f;
    uint32_t eps_expanded = ref_rms_bf16_broadcast_scalar_inplace_threaded(graph, graph.constant({1}, &eps, DType::FLOAT32), B, S, 1, false);
    uint32_t mean_sq_plus_eps = graph.add(mean_sq, eps_expanded);

    float half_val = 0.5f;
    uint32_t sqrt_exp = ref_rms_bf16_broadcast_scalar_inplace_threaded(graph, graph.constant({1}, &half_val, DType::FLOAT32), B, S, 1, false);
    uint32_t std_dev = graph.pow(mean_sq_plus_eps, sqrt_exp);

    float one_val = 1.0f;
    uint32_t one_node = ref_rms_bf16_broadcast_scalar_inplace_threaded(graph, graph.constant({1}, &one_val, DType::FLOAT32), B, S, 1, false);
    ...
    uint32_t one_full = ref_rms_bf16_broadcast_scalar_inplace_threaded(graph, graph.constant({1}, &one_val, DType::FLOAT32), B, S, D, true);
    uint32_t scale = graph.add(w_expanded, one_full);

    return graph.mul(x_norm, scale);
}

REGISTER_KERNEL_INPLACE("RMSNorm_BF16_B1_inplace_Threaded", 2, matchRMSNormBF16_3D_Inplace_Threaded, runRMSNormBF16_3D_Inplace_Threaded, refFactoryRMSNormBF16_inplace_Threaded, {Backend::CPU}, {DType::FLOAT32, DType::BF16}, {{1, 8, 640}, {640}}, {true, true}, {{Backend::CPU}, {Backend::CPU}});