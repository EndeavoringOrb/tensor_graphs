...
inline bool matchKreaModulatedRMSNorm(const std::vector<TensorNode> &inputs, const TensorNode &output)
{
...
inline void runKreaModulatedRMSNorm(const KernelContext &ctx)
{
    ...
    ThreadPool::get().parallel_for(num_threads, [=](uint32_t t) {
        ...
        for (uint32_t s = s_start; s < s_end; ++s)
        {
            ...
            for (uint32_t d = 0; d < D4; d += 4)
            {
                ...
                float32x4_t x_norm = vmulq_f32(vmulq_f32(vx, v_inv), vw);
                float32x4_t one_plus_scale = vaddq_f32(v_one, vscale);
                float32x4_t res = vfmaq_f32(vshift, one_plus_scale, x_norm);
            ...
            for (uint32_t d = 0; d < D4; d += 4)
            {
                ...
                {
                    uint32_t idx = d + k;
                    float x_norm = row_x[idx] * inv_std * w[idx];
                    row_out[idx] = (1.0f + row_scale[idx]) * x_norm + row_shift[idx];
                }
            }
#endif
            for (uint32_t d = D4; d < D; ++d)
            {
                float x_norm = row_x[d] * inv_std * w[d];
                row_out[d] = (1.0f + row_scale[d]) * x_norm + row_shift[d];
            }
        }
    });
}
...
REGISTER_KERNEL("Fused_Krea_Modulated_RMSNorm_F32", 4, 4, matchKreaModulatedRMSNorm, runKreaModulatedRMSNorm,
                refFactoryKreaModulatedRMSNorm, {0}, MemSpace(1, HandleType::CPP), {Engine(0, EngineType::CPU)},
