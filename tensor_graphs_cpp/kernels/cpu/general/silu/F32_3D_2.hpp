...
inline void runSiluF32_2(const std::vector<const void *> &inputs, const std::vector<void *> &outputs,
...
// ---------------------------------------------------------------------------
inline uint32_t ref_silu_broadcast_scalar_2(Graph &g, uint32_t scalar_id, const std::vector<uint32_t> &target_shape)
{
...
//   result   = mul(x, sig)
// ---------------------------------------------------------------------------
inline uint32_t refFactorySilu_2(const std::vector<uint32_t> &inputs, Graph &graph)
{
    ...
    // 3. den = 1 + exp(-x)
    float one_val = 1.0f;
    uint32_t one_node = ref_silu_broadcast_scalar_2(graph, graph.constant({1}, &one_val, DType::FLOAT32), target_shape);
...
REGISTER_KERNEL("Silu_3D_2", 1, matchSiluF32_2, runSiluF32_2, refFactorySilu_2,
                {Backend::CPU}, {DType::FLOAT32}, {{1, 4, 2048}}, {true}, {{Backend::CPU}});