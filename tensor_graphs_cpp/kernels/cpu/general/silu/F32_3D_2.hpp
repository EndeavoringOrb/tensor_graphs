...
inline LogicalId refFactorySilu_2(const std::vector<LogicalId> &inputs, Graph &graph)
{
    ...
    // 3. den = 1 + exp(-x)
    float one_val = 1.0f;
    LogicalId one_node = ref_silu_broadcast_scalar_2(graph, graph.constant({1}, &one_val, DType::FLOAT32), target_shape);
...
REGISTER_KERNEL("Silu_3D_2", 1, 1, matchSiluF32_2, runSiluF32_2, refFactorySilu_2, MemSpace(1, HandleType::CPP), {Engine(0, EngineType::CPU)}, {DType::FLOAT32}, {{1, 4, 2048}}, {true}, {{MemSpace(1, HandleType::CPP)}});