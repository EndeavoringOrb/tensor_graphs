...
// ---------------------------------------------------------------------------
inline LogicalId ref_silu_broadcast_scalar(Graph &g, LogicalId scalar_id, const std::vector<uint32_t> &target_shape)
{
...
inline LogicalId refFactorySilu(const std::vector<LogicalId> &inputs, Graph &graph)
{
    ...
    LogicalId e_node = ref_silu_broadcast_scalar(graph, graph.constant({1}, &e_val, DType::FLOAT32), target_shape);
    LogicalId exp_neg = graph.pow(e_node, neg_x);

    // 3. den = 1 + exp(-x)
    float one_val = 1.0f;
    LogicalId one_node = ref_silu_broadcast_scalar(graph, graph.constant({1}, &one_val, DType::FLOAT32), target_shape);
