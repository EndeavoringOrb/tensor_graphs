...
inline bool matchFlashAttentionGeneric3D(const std::vector<TensorNode> &inputs, const TensorNode &output)
{
...
inline void runFlashAttentionGeneric3D(const KernelContext &ctx)
{
    ...
    const uint32_t S = sQ[1];
    const uint32_t D = sQ[2];

    const uint64_t stride_B = static_cast<uint64_t>(S) * D;

    uint32_t num_threads = std::thread::hardware_concurrency();
    if (num_threads == 0)
        num_threads = 1;

    uint32_t total_tasks = B * S;
    ...
    ThreadPool::get().parallel_for(num_threads, [=](uint32_t t) {
        ...
        for (uint32_t task = start_task; task < end_task; ++task)
        {
            ...
            const float *q_row = Q_ptr + b * stride_B + s * D;
            const float *K_b = K_ptr + b * stride_B;
            const float *V_b = V_ptr + b * stride_B;
            float *o_row = O_ptr + b * stride_B + s * D;
