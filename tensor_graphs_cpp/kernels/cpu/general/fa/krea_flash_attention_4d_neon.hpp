...
inline bool matchKreaFlashAttention4D(const std::vector<TensorNode> &inputs, const TensorNode &output)
{
...
inline void runKreaFlashAttention4D(const KernelContext &ctx)
{
    ...
    // Cache-blocking parameters (fit within L1d / L2 cache)
    ...
    ThreadPool::get().parallel_for(num_threads, [=](uint32_t t) {
        ...
        for (uint32_t task = start_task; task < end_task; ++task)
        {
            ...
            for (uint32_t kb = 0; kb < num_k_blocks; ++kb)
            {
                ...
                // Step 2 & 3: Online Softmax Update & Output Accumulation
                for (uint32_t i = 0; i < cur_BR; ++i)
                {
                    ...
                    // Rescale accumulated O_row by alpha
#if defined(TG_HAS_NEON)
                    ...
                    // Accumulate P_blk @ V_blk into O_row
                    for (uint32_t j = 0; j < cur_BC; ++j)
                    {
                        float p_val = p_row[j];
                        const float *v_row = V_head + (k_start + j) * D;
#if defined(TG_HAS_NEON)
            ...
            // Final Normalization: O_row /= l_vec[i]
            for (uint32_t i = 0; i < cur_BR; ++i)
            {
...
inline LogicalId refFactoryKreaFlashAttention4D(const std::vector<LogicalId> &inputs, Graph &g)
{
...
REGISTER_KERNEL("Krea_Flash_Attention_4D_NEON", 3, 3, matchKreaFlashAttention4D, runKreaFlashAttention4D,
                refFactoryKreaFlashAttention4D, {}, MemSpace(1, HandleType::CPP), {Engine(0, EngineType::CPU)},
