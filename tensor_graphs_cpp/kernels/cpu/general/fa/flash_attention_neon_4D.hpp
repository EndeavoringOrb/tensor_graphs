...
inline bool matchFlashAttentionNeon(
                                 ...
                                 for (uint32_t blk = start_block; blk < end_block; ++blk)
                                 {
                                     ...
                                     for (uint32_t kb = 0; kb < num_k_blocks; ++kb)
                                     {
                                         ...
                                         for (uint32_t i = 0; i < BR; ++i)
                                         {
                                             ...
                                             // ---- PHASE 1: Q @ K^T (16 register Q-Cache mapped, latency decoupled) ----
...
// Pipeline K0 fully, then K1 fully ensuring 4+ cycle separation for accumulator latency
...
#define FMA_K1(idx, acc) acc = vfmaq_f32(acc, q##idx, vld1q_f32(k1_ptr + idx * 4));
                                             ...
                                             // ---- PHASE 2: Online Softmax (Max, Exp, P-gen & Normalization Scaling) ----
                                             ...
                                             // ---- PHASE 3: P @ V (16 register O-Cache mapped, ILP spaced across iter) ----
