...
inline bool matchPartialAddInplace(const std::vector<TensorNode> &inputs, const TensorNode &output)
{
    return inputs.size() == 6 && inputs[0].dtype == DType::FLOAT32 && inputs[0].storageType != StorageType::PERSISTENT;
}

inline uint32_t refFactoryPartialAdd(const std::vector<uint32_t> &inputs, Graph &graph)
{
    uint32_t target = inputs[0], A = inputs[1], B = inputs[2], st = inputs[3], en = inputs[4], step = inputs[5];
    uint32_t op_res = graph.add(graph.contiguous(graph.slice(A, st, en, step)), graph.contiguous(graph.slice(B, st, en, step)));
    return graph.scatter(target, graph.contiguous(op_res), st, en, step);
}

inline void runPartialAddInplace(const std::vector<const void *> &inputs, const std::vector<void *> &outputs,
...
REGISTER_KERNEL_INPLACE("Partial_Add_inplace", 6, matchPartialAddInplace, runPartialAddInplace, refFactoryPartialAdd, {Backend::CPU}, {DType::FLOAT32, DType::FLOAT32, DType::FLOAT32, DType::INT32, DType::INT32, DType::INT32}, {{8, 32}, {8, 32}, {8, 32}, {8}, {8}, {8}}, {false, false, false, false, false, false}, {{Backend::CPU}, {Backend::CPU}, {Backend::CPU}, {Backend::CPU}, {Backend::CPU}, {Backend::CPU}});