...
inline bool matchPartialMulInplace(const std::vector<TensorNode> &inputs, const TensorNode &output)
{
    return inputs.size() == 6 && inputs[0].dtype == DType::FLOAT32 && inputs[0].storageType != StorageType::PERSISTENT;
}

inline uint32_t refFactoryPartialMul(const std::vector<uint32_t> &inputs, Graph &graph)
{
    uint32_t target = inputs[0], A = inputs[1], B = inputs[2], st = inputs[3], en = inputs[4], step = inputs[5];
    uint32_t op_res = graph.mul(graph.contiguous(graph.slice(A, st, en, step)), graph.contiguous(graph.slice(B, st, en, step)));
    return graph.scatter(target, graph.contiguous(op_res), st, en, step);
}

inline void runPartialMulInplace(const std::vector<const void *> &inputs, const std::vector<void *> &outputs,
