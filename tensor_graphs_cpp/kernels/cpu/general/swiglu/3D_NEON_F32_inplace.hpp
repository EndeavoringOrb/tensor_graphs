...
inline LogicalId ref_swiglu_broadcast_scalar_Inplace(Graph &g, LogicalId scalar_id,
...
inline LogicalId refFactorySwiGLU_3D_NEON_Inplace(const std::vector<LogicalId> &inputs, Graph &graph)
{
    ...
    LogicalId e_node =
        ref_swiglu_broadcast_scalar_Inplace(graph, graph.constant({1}, &e_val, DType::FLOAT32), target_shape);
    LogicalId exp_neg = graph.pow(e_node, neg_x);

    // 3. den = 1 + exp(-x)
    float one_val = 1.0f;
    LogicalId one_node =
        ref_swiglu_broadcast_scalar_Inplace(graph, graph.constant({1}, &one_val, DType::FLOAT32), target_shape);
...
REGISTER_KERNEL("SwiGLU_3D_NEON_F32_Inplace", 2, 2, matchSwiGLU_3D_NEON_Inplace, runSwiGLU_3D_NEON_Inplace,
                refFactorySwiGLU_3D_NEON_Inplace, {0, 1}, MemSpace(1, HandleType::CPP), {Engine(0, EngineType::CPU)},
