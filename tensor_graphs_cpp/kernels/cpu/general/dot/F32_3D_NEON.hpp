 ...
 * Parallelization: Distributed across a 2D grid of M and N to eliminate memory
 * contention.
 */
...
inline void runDotF32_3D_Neon(const KernelContext &ctx)
{
    ...
    // Fast path uses aggressive NEON register caching but requires contiguous
    // inner dimension
    bool is_fast_path = (strideB_N == 1 && strideO_N == 1);

    if (is_fast_path)
    {
        // Grid dimensioning to perfectly balance M x N workloads across all 12
        // cores
        uint32_t n_split = 1;
        uint32_t m_split = 1;
                ...
                for (uint32_t b = 0; b < B_count; ++b)
                {
                    ...
                    for (uint32_t k_start = 0; k_start < K; k_start += K_BLK)
                    {
                        ...
                        for (; m + 3 < m_end; m += 4)
                        {
                            ...
                            // Highly tuned 4x16 Inner Microkernel (Uses 24 / 32 NEON
                            // registers entirely)
                            for (; n + 15 < n_end; n += 16)
                            {
    ...
    {
        // Slow Fallback (Preserves Original Correctness for odd/non-contiguous
        // matrices)
