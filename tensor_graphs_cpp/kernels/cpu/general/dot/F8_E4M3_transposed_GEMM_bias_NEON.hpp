...
inline bool matchF8E4M3TransposedGEMMBias(const std::vector<TensorNode> &inputs, const TensorNode &output)
{
    const auto &sX = inputs[0].getShape(); // [B, S, K]
    const auto &sW = inputs[1].getShape(); // [N, K]
    const auto &sB = inputs[2].getShape(); // [B, S, N] or broadcastable
    const auto &sO = output.getShape();    // [B, S, N]
    if (sX.size() != 3 || sW.size() != 2 || sO.size() != 3 || sB.size() != 3)
        return false;
    if (sX[2] != sW[1] || sO[2] != sW[0] || sB[2] != sW[0])
        return false;
    if (sO[0] != sX[0] || sO[1] != sX[1] || sB[0] != sX[0] || sB[1] != sX[1])
        return false;
    return isContiguous(output);
}

static inline float f8e4m3_bias_to_fp32_lut_val(uint8_t input)
{
...
static inline const float *get_f8e4m3_bias_gemm_lut()
{
    static const auto lut = []() {
        std::array<float, 256> table{};
        for (int i = 0; i < 256; ++i)
            table[i] = f8e4m3_bias_to_fp32_lut_val(static_cast<uint8_t>(i));
        return table;
    }();
    return lut.data();
}

inline void runF8E4M3TransposedGEMMBias(const KernelContext &ctx)
{
    const float *X = static_cast<const float *>(ctx.inputs[0]);
    const uint8_t *W = static_cast<const uint8_t *>(ctx.inputs[1]);
    const float *Bias = static_cast<const float *>(ctx.inputs[2]);
    ...
    const float *lut = get_f8e4m3_bias_gemm_lut();
    ...
    ThreadPool::get().parallel_for(num_threads, [=](uint32_t t) {
        ...
        for (uint32_t b = 0; b < B; ++b)
        {
            ...
            const float *batch_b = Bias + static_cast<uint64_t>(b) * S * N;
            ...
            for (uint32_t s = 0; s < S4; s += 4)
            {
                ...
                for (uint32_t n = n_start; n < N4; n += 4)
                {
                    ...
                    auto store_res = [&](uint32_t s_off, float32x4_t a0, float32x4_t a1, float32x4_t a2,
                                         float32x4_t a3) {
                        const float *b_row = batch_b + static_cast<uint64_t>(s + s_off) * N + n;
                        float *out_row = batch_out + static_cast<uint64_t>(s + s_off) * N + n;
                        float res[4] = {vaddvq_f32(a0) + b_row[0], vaddvq_f32(a1) + b_row[1], vaddvq_f32(a2) + b_row[2],
                                        vaddvq_f32(a3) + b_row[3]};
                    ...
                    for (uint32_t si = 0; si < 4; ++si)
                    {
                        ...
                        const float *b_row = batch_b + static_cast<uint64_t>(s + si) * N + n;
            ...
            for (uint32_t s = S4; s < S; ++s)
            {
                ...
                for (uint32_t n = n_start; n < N4; n += 4)
                {
                    ...
                    const float *b_row = batch_b + static_cast<uint64_t>(s) * N + n;
            ...
            for (uint32_t n = N4; n < n_end; ++n)
            {
                ...
                {
                    const float *x_row = batch_x + static_cast<uint64_t>(s) * K;
                    float sum = batch_b[static_cast<uint64_t>(s) * N + n];
                    for (uint32_t k = 0; k < K; ++k)
                    {
                        sum += x_row[k] * lut[w_row[k]];
                    }
...
REGISTER_KERNEL("F8_E4M3_Transposed_GEMM_Bias_NEON", 3, 3, matchF8E4M3TransposedGEMMBias, runF8E4M3TransposedGEMMBias,
                refFactoryF8E4M3TransposedGEMMBias, {2}, MemSpace(1, HandleType::CPP), {Engine(0, EngineType::CPU)},
                {DType::FLOAT32, DType::F8_E4M3, DType::FLOAT32}, {{1, 8, 64}, {1024, 64}, {1, 8, 1024}},
                {true, true, true},
