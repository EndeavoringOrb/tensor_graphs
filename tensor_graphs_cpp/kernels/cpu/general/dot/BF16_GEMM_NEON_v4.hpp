...
inline void runBF16GEMM_NEON_v4(const KernelContext &ctx)
{
    ...
    auto scalar_compute = [&](uint32_t m_s, uint32_t m_e, uint32_t n_s, uint32_t n_e, uint32_t k_s, uint32_t k_e,
                              bool accumulate) {
    ...
    if (K_main < K)
        scalar_compute(0, M_main, 0, N_main, K_main, K, true);
    if (M_main < M)
        scalar_compute(M_main, M, 0, N, 0, K, false);
    if (N_main < N)
        scalar_compute(0, M_main, N_main, N, 0, K, false);
}
...
REGISTER_KERNEL("BF16_GEMM_NEON_v4", 2, 2, matchBF16GEMM_NEON_v4, runBF16GEMM_NEON_v4, refFactoryBF16GEMM_NEON_v4,
