...
inline bool matchF8E4M3TransposedGEMMv3(const std::vector<TensorNode> &inputs, const TensorNode &output)
{
...
static inline float fp8_fast_e4m3_lut_val(uint8_t input)
{
...
static inline const float *get_fast_fp8_e4m3_lut()
{
    static const auto lut = []() {
        alignas(64) std::array<float, 256> table{};
        for (int i = 0; i < 256; ++i)
            table[i] = fp8_fast_e4m3_lut_val(static_cast<uint8_t>(i));
        return table;
    }();
    return lut.data();
}

inline void runF8E4M3TransposedGEMMv3(const KernelContext &ctx)
{
    ...
    const float *lut = get_fast_fp8_e4m3_lut();
    ...
    ThreadPool::get().parallel_for(num_threads, [=](uint32_t t) {
        ...
        std::vector<float> W_panel(N_PANEL * K);

        for (uint32_t n_chunk_start = n_start; n_chunk_start < n_end; n_chunk_start += N_PANEL)
        {
            uint32_t n_chunk_end = std::min(n_chunk_start + N_PANEL, n_end);
            uint32_t cur_N = n_chunk_end - n_chunk_start;

            // Unpack FP8 rows to FP32 into L1/L2 cache once per panel
            for (uint32_t i = 0; i < cur_N; ++i)
            {
                const uint8_t *src_w = W + static_cast<uint64_t>(n_chunk_start + i) * K;
                float *dst_w = W_panel.data() + static_cast<uint64_t>(i) * K;
            ...
            const uint32_t cur_N4 = cur_N & ~3u;
            const uint32_t M4 = M & ~3u;
            const uint32_t K4 = K & ~3u;

            for (uint32_t m = 0; m < M4; m += 4)
            {
                ...
                for (uint32_t n_off = 0; n_off < cur_N4; n_off += 4)
                {
                    const float *w0_ptr = W_panel.data() + static_cast<uint64_t>(n_off + 0) * K;
                    const float *w1_ptr = W_panel.data() + static_cast<uint64_t>(n_off + 1) * K;
                    const float *w2_ptr = W_panel.data() + static_cast<uint64_t>(n_off + 2) * K;
                    ...
                    for (uint32_t k = 0; k < K4; k += 4)
                    {
                        ...
                        float32x4_t wv0 = vld1q_f32(w0_ptr + k);
                        float32x4_t wv1 = vld1q_f32(w1_ptr + k);
                        float32x4_t wv2 = vld1q_f32(w2_ptr + k);
                        float32x4_t wv3 = vld1q_f32(w3_ptr + k);
                    ...
                    vst1q_f32(Out + static_cast<uint64_t>(m + 0) * N + n_chunk_start + n_off, r0);
                    vst1q_f32(Out + static_cast<uint64_t>(m + 1) * N + n_chunk_start + n_off, r1);
                    vst1q_f32(Out + static_cast<uint64_t>(m + 2) * N + n_chunk_start + n_off, r2);
                    vst1q_f32(Out + static_cast<uint64_t>(m + 3) * N + n_chunk_start + n_off, r3);
#else
                    for (uint32_t mi = 0; mi < 4; ++mi)
                    {
                        const float *x_row = X + static_cast<uint64_t>(m + mi) * K;
                        for (uint32_t ni = 0; ni < 4; ++ni)
                        {
                            const float *w_row = W_panel.data() + static_cast<uint64_t>(n_off + ni) * K;
                            float sum = 0.0f;
                            for (uint32_t k = 0; k < K; ++k)
                                sum += x_row[k] * w_row[k];
                            Out[static_cast<uint64_t>(m + mi) * N + n_chunk_start + n_off + ni] = sum;
                        }
                    }
#endif
                }
                ...
                // Inner N remainder
                for (uint32_t n_off = cur_N4; n_off < cur_N; ++n_off)
                {
                    const float *w_row = W_panel.data() + static_cast<uint64_t>(n_off) * K;
                    for (uint32_t mi = 0; mi < 4; ++mi)
                    {
                        const float *x_row = X + static_cast<uint64_t>(m + mi) * K;
                        float sum = 0.0f;
                        for (uint32_t k = 0; k < K; ++k)
                            sum += x_row[k] * w_row[k];
                        Out[static_cast<uint64_t>(m + mi) * N + n_chunk_start + n_off] = sum;
                    }
                }
            }

            // M remainder
            for (uint32_t m = M4; m < M; ++m)
            {
                const float *x_row = X + static_cast<uint64_t>(m) * K;
                for (uint32_t n_off = 0; n_off < cur_N; ++n_off)
                {
                    const float *w_row = W_panel.data() + static_cast<uint64_t>(n_off) * K;
                    float sum = 0.0f;
                    for (uint32_t k = 0; k < K; ++k)
                        sum += x_row[k] * w_row[k];
                    Out[static_cast<uint64_t>(m) * N + n_chunk_start + n_off] = sum;
                }
            }
        }
    });
...
inline LogicalId refFactoryF8E4M3TransposedGEMMv3(const std::vector<LogicalId> &inputs, Graph &graph)
{
...
REGISTER_KERNEL("F8_E4M3_Transposed_GEMM_NEON_v3", 2, 2, matchF8E4M3TransposedGEMMv3, runF8E4M3TransposedGEMMv3,
                refFactoryF8E4M3TransposedGEMMv3, {}, MemSpace(1, HandleType::CPP), {Engine(0, EngineType::CPU)},
