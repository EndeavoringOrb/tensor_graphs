...
static inline bool moe_v3_readFromFileAtOffset(int fd, uint64_t offset, void *buf, size_t bytes)
{
    if (bytes == 0)
        return true;
    ...
    while (remaining > 0)
    {
        ...
        DWORD toRead = static_cast<DWORD>(std::min<size_t>(remaining, 0x40000000ull));
        DWORD bytesRead = 0;
...
inline uint32_t refFactoryMoETopKFusedGEMM_StreamingStorage_v3(
...
REGISTER_KERNEL("MoE_TopK_FusedGEMM_Streaming_v3", 5, matchMoETopKFusedGEMM_v3, runMoETopKFusedGEMM_v3,
                refFactoryMoETopKFusedGEMM_StreamingStorage_v3,
                {Backend::CPU}, {DType::FLOAT32, DType::BF16, DType::BF16, DType::FLOAT32, DType::INT32},
                {{1, 8, 2048}, {256, 1024, 2048}, {256, 2048, 512}, {1, 8, 256}, {1, 8, 8}},
                {true, true, true, true, true}, {{Backend::CPU}, {Backend::CPU}, {Backend::CPU}, {Backend::CPU}, {Backend::CPU}});
#endif