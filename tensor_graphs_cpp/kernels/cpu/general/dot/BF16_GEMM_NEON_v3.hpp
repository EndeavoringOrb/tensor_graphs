...
inline void runBF16GEMM_NEON_v3(const std::vector<const void *> &inputs, const std::vector<void *> &outputs,
                                const std::vector<TensorView> &inViews, const std::vector<TensorView> &outViews)
{
    ...
    const uint32_t M = inViews[0].getShape()[1];
    ...
    // L2 Cache blocking dimension for N. 256 columns of W (K=3072) is exactly 1.57 MB,
    // fitting perfectly inside L2/SLC cache alongside X.
    ...
    for (uint32_t t = 0; t < num_threads; ++t)
    {
        workers.emplace_back([=]()
        {
            ...
            if (m_start >= total_M) return;
            uint32_t m_end = std::min(m_start + m_block, total_M);
            uint32_t m_rem = m_end & ~3;

            // Thread-local panel buffer for W. Allocated once per thread to avoid OS mmap overhead.
            // Layout is packed as [N_BLOCK/16, K, 16] to enable 100% contiguous SIMD access in the inner loop.
            ...
            for (uint32_t n_outer = 0; n_outer < N; n_outer += N_BLOCK) {
                ...
                // Outer loop K, inner loop N_chunk ensures W is read sequentially along rows.
                // Eliminates 55KB stride jumps, achieving 100% cache line utilization and 0 TLB misses.
                if (n_curr_rem > 0) {
                ...
                for (uint32_t m = m_start; m < m_rem; m += 4) {
                    for (uint32_t n_chunk = 0; n_chunk < n_curr_rem; n_chunk += 16) {
                        ...
                        for (uint32_t k = 0; k < k_rem; k += 2) {
                            // Unroll k by 2. Consecutive vld1q_u16 fuse into AArch64 'ldp' (load pair).
                            ...
                            // FMAs for k + 0
                            c00 = vfmaq_f32(c00, x0_0, w0_0); c01 = vfmaq_f32(c01, x0_0, w1_0);
                            c02 = vfmaq_f32(c02, x0_0, w2_0); c03 = vfmaq_f32(c03, x0_0, w3_0);

                            c10 = vfmaq_f32(c10, x1_0, w0_0); c11 = vfmaq_f32(c11, x1_0, w1_0);
                            c12 = vfmaq_f32(c12, x1_0, w2_0); c13 = vfmaq_f32(c13, x1_0, w3_0);

                            c20 = vfmaq_f32(c20, x2_0, w0_0); c21 = vfmaq_f32(c21, x2_0, w1_0);
                            c22 = vfmaq_f32(c22, x2_0, w2_0); c23 = vfmaq_f32(c23, x2_0, w3_0);

                            c30 = vfmaq_f32(c30, x3_0, w0_0); c31 = vfmaq_f32(c31, x3_0, w1_0);
                            c32 = vfmaq_f32(c32, x3_0, w2_0); c33 = vfmaq_f32(c33, x3_0, w3_0);

                            // FMAs for k + 1
                            c00 = vfmaq_f32(c00, x0_1, w0_1); c01 = vfmaq_f32(c01, x0_1, w1_1);
                            c02 = vfmaq_f32(c02, x0_1, w2_1); c03 = vfmaq_f32(c03, x0_1, w3_1);

                            c10 = vfmaq_f32(c10, x1_1, w0_1); c11 = vfmaq_f32(c11, x1_1, w1_1);
                            c12 = vfmaq_f32(c12, x1_1, w2_1); c13 = vfmaq_f32(c13, x1_1, w3_1);

                            c20 = vfmaq_f32(c20, x2_1, w0_1); c21 = vfmaq_f32(c21, x2_1, w1_1);
                            c22 = vfmaq_f32(c22, x2_1, w2_1); c23 = vfmaq_f32(c23, x2_1, w3_1);

                            c30 = vfmaq_f32(c30, x3_1, w0_1); c31 = vfmaq_f32(c31, x3_1, w1_1);
                            c32 = vfmaq_f32(c32, x3_1, w2_1); c33 = vfmaq_f32(c33, x3_1, w3_1);
                        }
                        ...
                        for (uint32_t k = k_rem; k < K; ++k) {
                            ...
                            c00 = vfmaq_f32(c00, x0, w0); c01 = vfmaq_f32(c01, x0, w1);
                            c02 = vfmaq_f32(c02, x0, w2); c03 = vfmaq_f32(c03, x0, w3);

                            c10 = vfmaq_f32(c10, x1, w0); c11 = vfmaq_f32(c11, x1, w1);
                            c12 = vfmaq_f32(c12, x1, w2); c13 = vfmaq_f32(c13, x1, w3);

                            c20 = vfmaq_f32(c20, x2, w0); c21 = vfmaq_f32(c21, x2, w1);
                            c22 = vfmaq_f32(c22, x2, w2); c23 = vfmaq_f32(c23, x2, w3);

                            c30 = vfmaq_f32(c30, x3, w0); c31 = vfmaq_f32(c31, x3, w1);
                            c32 = vfmaq_f32(c32, x3, w2); c33 = vfmaq_f32(c33, x3, w3);
                        }

                        uint32_t n_actual = n_outer + n_chunk;
                        float* out_ptr = Out + m * N + n_actual;
                        vst1q_f32(out_ptr + 0 * N + 0, c00); vst1q_f32(out_ptr + 0 * N + 4, c01);
                        vst1q_f32(out_ptr + 0 * N + 8, c02); vst1q_f32(out_ptr + 0 * N + 12, c03);

                        vst1q_f32(out_ptr + 1 * N + 0, c10); vst1q_f32(out_ptr + 1 * N + 4, c11);
                        vst1q_f32(out_ptr + 1 * N + 8, c12); vst1q_f32(out_ptr + 1 * N + 12, c13);

                        vst1q_f32(out_ptr + 2 * N + 0, c20); vst1q_f32(out_ptr + 2 * N + 4, c21);
                        vst1q_f32(out_ptr + 2 * N + 8, c22); vst1q_f32(out_ptr + 2 * N + 12, c23);

                        vst1q_f32(out_ptr + 3 * N + 0, c30); vst1q_f32(out_ptr + 3 * N + 4, c31);
                        vst1q_f32(out_ptr + 3 * N + 8, c32); vst1q_f32(out_ptr + 3 * N + 12, c33);
                    }

                    // Handle remaining N columns in the current block (if N_BLOCK is not a multiple of 16)
...
inline uint32_t refFactoryBF16GEMM_NEON_v3(const std::vector<uint32_t> &inputs, Graph &graph)
{
    uint32_t w_f32 = graph.cast(inputs[1], DType::FLOAT32);
    return graph.dot(inputs[0], w_f32);
}

REGISTER_KERNEL("BF16_GEMM_NEON_v3", 2, matchBF16GEMM_NEON_v3, runBF16GEMM_NEON_v3, refFactoryBF16GEMM_NEON_v3, {Backend::CPU}, {DType::FLOAT32, DType::BF16}, {{1, 8, 64}, {1, 64, 1024}}, {true, true}, {{Backend::CPU}, {Backend::CPU}});
#endif