...
inline void runBF16GEMM_NEON_v2(const std::vector<const void *> &inputs, const std::vector<void *> &outputs,
                                const std::vector<TensorView> &inViews, const std::vector<TensorView> &outViews)
{
    ...
    for (uint32_t t = 0; t < num_threads; ++t)
    {
        workers.emplace_back([=]()
            ...
            // Transpose/pack a vertical slice of W to avoid extreme TLB/cache thrashing in the inner loop.
            std::vector<uint16_t> w_packed(K * 16);

            // Re-order loops: Iterate N -> pack W slice -> iterate M -> iterate K (contiguous memory loads)
            for (uint32_t n = 0; n < n_rem; n += 16) {
                
                // Pack 16 columns of W across all K sequentially to stay in L1 cache bounds
                ...
                for (uint32_t m = m_start; m < m_rem; m += 4) {
                    ...
                    for (uint32_t k = 0; k < K; ++k) {
                        // All accesses in inner loop are now contiguous and completely prefetchable
                    ...
                    vst1q_f32(out_ptr0 + 0, c00); vst1q_f32(out_ptr0 + 4, c01);
                    vst1q_f32(out_ptr0 + 8, c02); vst1q_f32(out_ptr0 + 12, c03);

                    vst1q_f32(out_ptr1 + 0, c10); vst1q_f32(out_ptr1 + 4, c11);
                    vst1q_f32(out_ptr1 + 8, c12); vst1q_f32(out_ptr1 + 12, c13);

                    vst1q_f32(out_ptr2 + 0, c20); vst1q_f32(out_ptr2 + 4, c21);
                    vst1q_f32(out_ptr2 + 8, c22); vst1q_f32(out_ptr2 + 12, c23);

                    vst1q_f32(out_ptr3 + 0, c30); vst1q_f32(out_ptr3 + 4, c31);
                    vst1q_f32(out_ptr3 + 8, c32); vst1q_f32(out_ptr3 + 12, c33);
                }
            }

            // Handle remaining N for main M block (unlikely required given shape dimensions)
            for (uint32_t m = m_start; m < m_rem; m += 4) {
...
inline uint32_t refFactoryBF16GEMM_NEON_v2(const std::vector<uint32_t> &inputs, Graph &graph)
{
...
REGISTER_KERNEL("BF16_GEMM_NEON_v2", 2, matchBF16GEMM_NEON_v2, runBF16GEMM_NEON_v2, refFactoryBF16GEMM_NEON_v2, {Backend::CPU}, {DType::FLOAT32, DType::BF16}, {{1, 8, 64}, {1, 64, 1024}}, {true, true}, {{Backend::CPU}, {Backend::CPU}});
#endif