...
inline void runBF16TransposedGEMM_v4(const KernelContext &ctx)
{
    ...
    n_block = (n_block + 3) & ~3; // Ensure n_block is a multiple of 4
                ...
                for (uint32_t n = n_start; n + 4 <= n_end; n += 4) {
                    ...
                    for (uint32_t s = 0; s < (S & ~3); s += 4) {
                        ...
                        for (uint32_t k = 0; k < (K & ~3); k += 4) {
                            ...
                            // Load 4 FP32 values for each of the 4 S-rows
...
REGISTER_KERNEL("BF16_Transposed_GEMM_NEON_v4", 2, 2, matchBF16TransposedGEMM_v4, runBF16TransposedGEMM_v4, refFactoryBF16TransposedGEMM_v4, MemSpace(1, HandleType::CPP), {Engine(0, EngineType::CPU)}, {DType::FLOAT32, DType::BF16}, {{1, 8, 64}, {1024, 64}}, {true, true}, {{MemSpace(1, HandleType::CPP)}, {MemSpace(1, HandleType::CPP)}});
