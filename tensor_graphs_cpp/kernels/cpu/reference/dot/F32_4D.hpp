...
{
    const float *A = static_cast<const float *>(inputs[0]); // [B, H, M, K]
    const float *B = static_cast<const float *>(inputs[1]); // [B, H, K, N]
    float *C = static_cast<float *>(outputs[0]);            // [B, H, M, N]
