...
inline bool matchCopy_OpenCL_CPU(const std::vector<TensorNode> &inputs, const TensorNode &output)
{
    return inputs[0].getShape() == output.getShape() && inputs[0].strides == output.strides &&
           inputs[0].dtype == output.dtype && isContiguous(output);
}
...
inline void runCopy_OpenCL_CPU(const KernelContext &ctx)
{
    ...
    // Enqueue a blocking read. The driver handles both synchronization and cache
    // invalidation.
    cl_int err = clEnqueueReadBuffer(OpenCLState::get().queue, src_device_buf,
                                     CL_TRUE, // Blocking read
                                     0, size_bytes, dst_host_ptr, 0, nullptr, nullptr);
