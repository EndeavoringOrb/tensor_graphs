...
inline bool matchCopyTo_CPU_CUDA(const std::vector<TensorNode> &inputs, const TensorNode &output)
{
    ...
    return (in.dtype == output.dtype && in.getShape() == output.getShape() && in.strides == output.strides);
}
...
REGISTER_REF_KERNEL(OpType::COPY_TO, 1, matchCopyTo_CPU_CUDA, runCopyTo_CPU_CUDA, {Backend::CUDA}, {DType::ANY}, {{8, 32}}, {false}, {{Backend::CPU}});
#endif