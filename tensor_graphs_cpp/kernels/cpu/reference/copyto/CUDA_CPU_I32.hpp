...
inline bool matchCopyTo_CUDA_CPU_I32(const std::vector<TensorNode> &inputs, const TensorNode &output, const std::unordered_map<uint32_t, uint32_t> &refCounts)
{
    if (inputs.size() != 1)
        return false;
                                   ...
                                   const std::vector<TensorView> &inViews, const std::vector<TensorView> &outViews)
{
    ...
    for (uint64_t i = 0; i < numElements; ++i)
    {
        ...
        if (err != cudaSuccess)
        {
            Error:throw_error(std::string("CUDA strided copyto (I32) device->host failed: ") + cudaGetErrorString(err));
        }
    }
}

REGISTER_REF_KERNEL(OpType::COPY_TO, Backend::CPU, matchCopyTo_CUDA_CPU_I32, runCopyTo_CUDA_CPU_I32);
#endif