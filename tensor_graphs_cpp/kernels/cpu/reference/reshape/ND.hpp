                         ...
                         const std::vector<TensorView> &inViews, const std::vector<TensorView> &outViews)
{
    ...
    {
        uint64_t src_idx = getStridedIndex(i, inViews[0].getShape(), inViews[0].strides);
        uint64_t dst_idx = getStridedIndex(i, outViews[0].getShape(), outViews[0].strides);
        std::memcpy(dst + dst_idx * elementSize, src + src_idx * elementSize, elementSize);
    }
}

REGISTER_REF_KERNEL(OpType::RESHAPE, matchReshapeND, runReshapeND, {Backend::CPU});