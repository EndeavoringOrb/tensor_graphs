...
{
    // Axis tensor is input 0. We need at least one data tensor + axis.
    return true;
}

inline void runConcatI32_ND(const KernelContext &ctx)
{
    ...
    for (uint64_t i = 0; i < totalElements; ++i)
    {
        ...
        // Data tensors are in indices 1 to inputs.size() - 1
        ...
        const int32_t *in_ptr = static_cast<const int32_t *>(ctx.inputs[n + 1]);
