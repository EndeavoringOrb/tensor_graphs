...
inline bool matchConcatF32_ND(const std::vector<TensorNode> &inputs, const TensorNode &output, const std::unordered_map<uint32_t, uint32_t> &refCounts)
{
    ...
    for (size_t i = 0; i < inputs.size() - 1; ++i) {
        if (!isContiguous(inputs[i])) return false;
    }
                            ...
                            const std::vector<TensorView> &inViews, const std::vector<TensorView> &outViews)
{
    ...
    if (axis < 0)
        axis += static_cast<int32_t>(outViews[0].getShape().size());

    uint64_t outer = 1, inner = 1;
    for (int d = 0; d < axis; ++d)
        outer *= outViews[0].getShape()[d];
    for (size_t d = axis + 1; d < outViews[0].getShape().size(); ++d)
        inner *= outViews[0].getShape()[d];
    ...
    for (uint64_t outer_idx = 0; outer_idx < outer; ++outer_idx)
    {
        for (size_t n = 0; n < inputs.size() - 1; ++n)
        {
            ...
            uint64_t chunk = static_cast<uint64_t>(inViews[n].getShape()[axis]) * inner;
...
REGISTER_REF_KERNEL(OpType::CONCAT, 2, matchConcatF32_ND, runConcatF32_ND, {Backend::CPU}, {DType::FLOAT32, DType::INT32}, {{8, 32}, {8, 32}}, {false, false});