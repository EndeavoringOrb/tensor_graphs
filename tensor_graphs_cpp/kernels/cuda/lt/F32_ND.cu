...
__global__ void lt_f32_nd_kernel(const float* A, const float* B, bool* Out, uint64_t n) {
...
inline bool matchLtF32_CUDA_ND(const std::vector<TensorNode> &inputs, const TensorNode &output) {
...
inline void runLtF32_CUDA_ND(const KernelContext &ctx) {
    ...
    lt_f32_nd_kernel<<<numBlocks, blockSize, 0, stream>>>(A, B, Out, n);

    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) {
        Error::throw_err("CUDA kernel launch failed in Lt_F32_CUDA_ND: " + std::string(cudaGetErrorString(err)));
    }
}
