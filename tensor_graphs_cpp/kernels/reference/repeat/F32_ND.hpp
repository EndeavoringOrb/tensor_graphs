...
inline void runRepeatF32_ND(const std::vector<const void *> &inputs, const std::vector<void *> &outputs,
    ...
    // Handle Identity / In-place optimizations safely
    if (src == dst && repeats == 1) return;

    int32_t ndim = static_cast<int32_t>(inViews[0].shape.size());
    if (axis < 0) axis += ndim;

    uint64_t outer_size = 1;
    for (int32_t i = 0; i < axis; ++i) outer_size *= inViews[0].shape[i];

    uint64_t inner_size = 1;
    for (int32_t i = axis + 1; i < ndim; ++i) inner_size *= inViews[0].shape[i];
...
REGISTER_KERNEL_INPLACE(OpType::REPEAT, Backend::CPU, matchRepeatF32_ND, runRepeatF32_ND);