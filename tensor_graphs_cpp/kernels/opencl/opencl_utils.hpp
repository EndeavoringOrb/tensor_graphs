...
namespace OpenCL {
    ...
    inline void setArgSVM(cl_kernel k, cl_uint index, const void* ptr) {
        cl_int err = clSetKernelArgSVMPointer(k, index, ptr);
        if (err != CL_SUCCESS) {
            Error::throw_err("OpenCL: Failed to set SVM kernel arg " + std::to_string(index));
        }
    }
}