...
class FluxVAEDecoder : public FluxGraphBase
{
  private:
    ...
    uint32_t group_norm_atomic(uint32_t x, const std::string &w_name, const std::string &b_name, int N, int C, int H,
                               int W)
    {
        int G = 32;
        int M = (C / G) * H * W;
        ...
        uint32_t x_norm = g.reshape(g.mul(x_sub, repeat_ax(g.div(expand_scalar_to_3d(1.0f, N, G, 1), std), M, 2)),
                                    g.constant({4}, std::vector<int32_t>{N, C, H, W}.data(), DType::INT32));

        int32_t w_sh[] = {1, C, 1, 1};
        uint32_t w_4d = repeat_ax(
            repeat_ax(repeat_ax(g.reshape(weight(w_name), g.constant({4}, w_sh, DType::INT32)), N, 0), H, 2), W, 3);
        uint32_t b_4d = repeat_ax(
            repeat_ax(repeat_ax(g.reshape(weight(b_name), g.constant({4}, w_sh, DType::INT32)), N, 0), H, 2), W, 3);

        return g.add(g.mul(x_norm, w_4d), b_4d);
    }

    uint32_t conv2d_atomic(uint32_t x, const std::string &w_name, const std::string &b_name, int kernel, int stride,
                           int padding, int H, int W)
    {
        ...
        // Use the original INPUT node's shape directly since CAST doesn't inherit
        // it during build time
        ...
        // Weight Reshape: [1, C_out, K] // TODO: add dot kernel for 2D dot so we
        // don't have to add batch dimension
    ...
    uint32_t upsample2x_atomic(uint32_t x, int C, int H, int W)
    {
        ...
        // 1. Upsample Height: [1, C, H, W] -> [1, C, H, 1, W] -> [1, C, H, 2, W] ->
        // [1, C, 2H, W]
        int32_t sh_h_pre[] = {1, (int32_t)C, (int32_t)H, 1, (int32_t)W};
        int32_t ax3_val = 3;
        uint32_t h_rep = g.repeat(g.reshape(x, g.constant({5}, sh_h_pre, DType::INT32)), r_node,
                                  g.constant({1}, &ax3_val, DType::INT32));
        ...
        // 2. Upsample Width: [1, C, 2H, W] -> [1, C, 2H, W, 1] -> [1, C, 2H, W, 2]
        // -> [1, C, 2H, 2W]
        int32_t sh_w_pre[] = {1, (int32_t)C, (int32_t)H * 2, (int32_t)W, 1};
        int32_t ax4_val = 4;
        uint32_t w_rep = g.repeat(g.reshape(h_res, g.constant({5}, sh_w_pre, DType::INT32)), r_node,
                                  g.constant({1}, &ax4_val, DType::INT32));
    ...
    uint32_t attnblock(uint32_t x, const std::string &pfx, int C, int H, int W)
    {
        ...
        auto flat = [&](uint32_t t) {
            ...
            return g.reshape(g.contiguous(g.permute(t, g.constant({4}, p, DType::INT32))),
                             g.constant({3}, sh3, DType::INT32));
        };

        int32_t p_k[] = {0, 2, 1};
        uint32_t scores = g.mul(g.dot(flat(q), g.contiguous(g.permute(flat(k), g.constant({3}, p_k, DType::INT32)))),
                                expand_scalar_to_3d(1.0f / std::sqrt((float)C), 1, H * W, H * W));
        ...
        out = conv2d_atomic(g.contiguous(g.permute(g.reshape(out, g.constant({4}, sh4, DType::INT32)),
                                                   g.constant({4}, p_out, DType::INT32))),
                            pfx + ".to_out.0.weight", pfx + ".to_out.0.bias", 1, 1, 0, H, W);
        return g.add(out, x);
    }

  public:
    FluxVAEDecoder(FluxConfig config, Graph &graph, MemoryManager &memory, const std::string &weight_path, uint32_t h,
                   uint32_t w)
    ...
    uint32_t build_graph(uint32_t latent)
    {
        ...
        uint32_t mu = repeat_ax(
            repeat_ax(g.reshape(weight("bn.running_mean"), g.constant({4}, b_sh, DType::INT32)), initial_h, 2),
            initial_w, 3);
        uint32_t var =
            repeat_ax(repeat_ax(g.reshape(weight("bn.running_var"), g.constant({4}, b_sh, DType::INT32)), initial_h, 2),
                      initial_w, 3);

        uint32_t std = g.pow(g.add(var, expand_scalar_to_4d(1e-4f, 1, cfg.latent_channels, initial_h, initial_w)),
                             expand_scalar_to_4d(0.5f, 1, cfg.latent_channels, initial_h, initial_w));
        uint32_t h = g.add(g.mul(latent, std), mu);

        int32_t sh1[] = {1,
                         (int32_t)cfg.vae_z_channels,
                         (int32_t)cfg.patch_size,
                         (int32_t)cfg.patch_size,
                         (int32_t)initial_h,
                         (int32_t)initial_w};
        ...
        h = g.reshape(g.contiguous(g.permute(g.reshape(h, g.constant({6}, sh1, DType::INT32)),
                                             g.constant({6}, p_u, DType::INT32))),
                      g.constant({4}, sh2, DType::INT32));
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            for (int r = 0; r < 3; ++r)
            {
                ...
                // A shortcut is needed if the input channels to this block don't match
                // output
            ...
            if (i < 3) // Upsample between blocks (but not after the last one)
            {
                ...
                h = conv2d_atomic(h, "decoder.up_blocks." + std::to_string(i) + ".upsamplers.0.conv.weight",
                                  "decoder.up_blocks." + std::to_string(i) + ".upsamplers.0.conv.bias", 3, 1, 1, curr_h,
                                  curr_w);
            }
        }

        // Final layers
        h = group_norm_atomic(h, "decoder.conv_norm_out.weight", "decoder.conv_norm_out.bias", 1, current_channels,
                              curr_h, curr_w);
