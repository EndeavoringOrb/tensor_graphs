...
class FluxTransformer : public FluxGraphBase
{
  private:
    ...
    */
    uint32_t timestep_embedding(uint32_t t_in, uint32_t dim)
    {
        ...
        uint32_t freqs = g.cast(g.arange(g.constant({1}, &start, DType::INT32), g.constant({1}, &half, DType::INT32),
    ...
    uint32_t layer_norm_atomic(uint32_t x, uint32_t seq)
    {
        ...
        uint32_t mean = repeat_ax(g.div(g.sum(x, ax_node), expand_scalar_to_3d((float)cfg.hidden_size, 1, seq, 1)),
                                  cfg.hidden_size, 2);
    ...
    uint32_t rms_norm_atomic(uint32_t x, const std::string &w_name, uint32_t seq, uint32_t num_heads, uint32_t head_dim)
    {
        ...
        uint32_t std = g.pow(g.add(mean_sq, expand_scalar_to_4d(1e-6f, 1, num_heads, seq, 1)),
                             expand_scalar_to_4d(0.5f, 1, num_heads, seq, 1));
    ...
    uint32_t apply_rope_2d_consecutive(uint32_t x, uint32_t cos, uint32_t sin, uint32_t seq)
    {
        ...
        uint32_t x_even = g.slice(x, g.constant({4}, starts1, DType::INT32), g.constant({4}, ends1, DType::INT32),
                                  g.constant({4}, steps, DType::INT32));

        int32_t starts_odd[] = {0, 0, 0, 1};
        uint32_t x_odd = g.slice(x, g.constant({4}, starts_odd, DType::INT32), g.constant({4}, ends1, DType::INT32),
                                 g.constant({4}, steps, DType::INT32));

        int32_t ends_cos[] = {1, 1, (int32_t)seq, (int32_t)cfg.head_dim};
        uint32_t c = repeat_ax(g.slice(cos, g.constant({4}, starts1, DType::INT32),
                                       g.constant({4}, ends_cos, DType::INT32), g.constant({4}, steps, DType::INT32)),
                               cfg.num_heads, 1);
        uint32_t s = repeat_ax(g.slice(sin, g.constant({4}, starts1, DType::INT32),
                                       g.constant({4}, ends_cos, DType::INT32), g.constant({4}, steps, DType::INT32)),
                               cfg.num_heads, 1);
        ...
        int32_t ax4 = 4;
        uint32_t stacked = g.concat({g.reshape(out_even, sh5_node), g.reshape(out_odd, sh5_node)},
                                    g.constant({1}, &ax4, DType::INT32));
    ...
    std::vector<uint32_t> compute_mods(uint32_t t_emb, const std::string &w_name, int chunks)
    {
        ...
        for (int i = 0; i < chunks; ++i)
        {
            ...
            int32_t steps[] = {1, 1, 1};
            uint32_t out = g.slice(mod, g.constant({3}, starts, DType::INT32), g.constant({3}, ends, DType::INT32),
  ...
  public:
    FluxTransformer(FluxConfig config, Graph &graph, MemoryManager &memory, const std::string &weight_path, uint32_t h,
                    uint32_t w)
    ...
    */
    uint32_t build_graph(uint32_t img_latent, uint32_t txt_emb, uint32_t timestep, uint32_t rope_cos, uint32_t rope_sin)
    {
        ...
        uint32_t t_emb_raw = linear(g.reshape(t_sincos, g.constant({3}, sh3_t, DType::INT32)),
                                    "time_guidance_embed.timestep_embedder.linear_1.weight", 256,
                                    cfg.hidden_size); // (1, 1, 256) @ (256, 3072) -> (1, 1, 3072) F32
        uint32_t t_emb_mid = silu_atomic(t_emb_raw, 1, 1, cfg.hidden_size); // (1, 1, 3072) F32
        uint32_t t_emb_out = linear(t_emb_mid, "time_guidance_embed.timestep_embedder.linear_2.weight", cfg.hidden_size,
                                    cfg.hidden_size); // (1, 1, 3072) @ (3072, 3072) -> (1, 1, 3072) F32
        uint32_t t_emb_silu = silu_atomic(t_emb_out, 1, 1, cfg.hidden_size); // (1, 1, 3072) F32

        // NLC
        int32_t p_img[] = {0, 2, 3, 1};
        uint32_t img_perm =
            g.permute(img_latent, g.constant({4}, p_img,
                                             DType::INT32)); // (1, latent_h, latent_w, cfg.latent_channels) F32
        ...
        uint32_t img_hidden = linear(g.reshape(img_perm, g.constant({3}, sh_img, DType::INT32)), "x_embedder.weight",
                                     cfg.latent_channels, cfg.hidden_size);
        ...
        uint32_t txt_cos =
            g.slice(rope_cos, g.constant({4}, slice_t_s, DType::INT32), g.constant({4}, slice_txt_e, DType::INT32),
                    g.constant({4}, slice_step, DType::INT32));
        uint32_t txt_sin =
            g.slice(rope_sin, g.constant({4}, slice_t_s, DType::INT32), g.constant({4}, slice_txt_e, DType::INT32),
                    g.constant({4}, slice_step, DType::INT32));

        int32_t slice_img_s[] = {0, 0, (int32_t)cfg.text_max_seq, 0};
        int32_t slice_img_e[] = {1, 1, (int32_t)total_seq_len, (int32_t)cfg.head_dim};
        uint32_t img_cos =
            g.slice(rope_cos, g.constant({4}, slice_img_s, DType::INT32), g.constant({4}, slice_img_e, DType::INT32),
                    g.constant({4}, slice_step, DType::INT32));
        uint32_t img_sin =
            g.slice(rope_sin, g.constant({4}, slice_img_s, DType::INT32), g.constant({4}, slice_img_e, DType::INT32),
                    g.constant({4}, slice_step, DType::INT32));
        ...
        for (uint32_t i = 0; i < cfg.num_double_layers; ++i)
        {
            ...
            uint32_t img_mod =
                apply_mod(layer_norm_atomic(img_hidden, img_seq_len), mod_img_all[0], mod_img_all[1], img_seq_len);
            uint32_t txt_mod = apply_mod(layer_norm_atomic(txt_hidden, cfg.text_max_seq), mod_txt_all[0],
                                         mod_txt_all[1], cfg.text_max_seq);

            auto reshape_for_attn = [&](uint32_t x, int L) {
                int32_t sh4[] = {1, L, (int32_t)cfg.num_heads, (int32_t)cfg.head_dim};
                int32_t p[] = {0, 2, 1, 3};
                return g.permute(g.reshape(g.contiguous(x), g.constant({4}, sh4, DType::INT32)),
                                 g.constant({4}, p, DType::INT32));
            };

            uint32_t img_q = apply_rope_2d_consecutive(
                rms_norm_atomic(
                    reshape_for_attn(linear(img_mod, p + ".attn.to_q.weight", cfg.hidden_size, cfg.hidden_size),
                                     img_seq_len),
                    p + ".attn.norm_q.weight", img_seq_len, cfg.num_heads, cfg.head_dim),
                img_cos, img_sin, img_seq_len);
            uint32_t img_k = apply_rope_2d_consecutive(
                rms_norm_atomic(
                    reshape_for_attn(linear(img_mod, p + ".attn.to_k.weight", cfg.hidden_size, cfg.hidden_size),
                                     img_seq_len),
                    p + ".attn.norm_k.weight", img_seq_len, cfg.num_heads, cfg.head_dim),
                img_cos, img_sin, img_seq_len);
            uint32_t img_v = reshape_for_attn(
                linear(img_mod, p + ".attn.to_v.weight", cfg.hidden_size, cfg.hidden_size), img_seq_len);

            uint32_t txt_q = apply_rope_2d_consecutive(
                rms_norm_atomic(
                    reshape_for_attn(linear(txt_mod, p + ".attn.add_q_proj.weight", cfg.hidden_size, cfg.hidden_size),
                                     cfg.text_max_seq),
                    p + ".attn.norm_added_q.weight", cfg.text_max_seq, cfg.num_heads, cfg.head_dim),
                txt_cos, txt_sin, cfg.text_max_seq);
            uint32_t txt_k = apply_rope_2d_consecutive(
                rms_norm_atomic(
                    reshape_for_attn(linear(txt_mod, p + ".attn.add_k_proj.weight", cfg.hidden_size, cfg.hidden_size),
                                     cfg.text_max_seq),
                    p + ".attn.norm_added_k.weight", cfg.text_max_seq, cfg.num_heads, cfg.head_dim),
                txt_cos, txt_sin, cfg.text_max_seq);
            uint32_t txt_v = reshape_for_attn(
                linear(txt_mod, p + ".attn.add_v_proj.weight", cfg.hidden_size, cfg.hidden_size), cfg.text_max_seq);
            ...
            auto attn = [&](uint32_t q, uint32_t k, uint32_t v, int L_q) {
                int32_t p_k[] = {0, 1, 3, 2};
                uint32_t scores = g.mul(
                    g.dot(q, g.contiguous(g.permute(k, g.constant({4}, p_k, DType::INT32)))),
                    expand_scalar_to_4d(1.0f / std::sqrt((float)cfg.head_dim), 1, cfg.num_heads, L_q, total_seq_len));
                int32_t ax = -1;
                uint32_t shifted = g.add(
                    scores, g.neg(repeat_ax(g.max(scores, g.constant({1}, &ax, DType::INT32)), total_seq_len, 3)));
                uint32_t exps = g.pow(expand_scalar_to_4d(2.7182818f, 1, cfg.num_heads, L_q, total_seq_len), shifted);
                uint32_t probs =
                    g.div(exps, repeat_ax(g.sum(exps, g.constant({1}, &ax, DType::INT32)), total_seq_len, 3));
            ...
            uint32_t img_attn_out = linear(attn(img_q, joint_k, joint_v, img_seq_len), p + ".attn.to_out.0.weight",
                                           cfg.hidden_size, cfg.hidden_size);
            uint32_t txt_attn_out = linear(attn(txt_q, joint_k, joint_v, cfg.text_max_seq),
                                           p + ".attn.to_add_out.weight", cfg.hidden_size, cfg.hidden_size);

            int32_t sh3_gate[] = {1, 1, (int32_t)cfg.hidden_size};
            img_hidden = g.add(
                img_hidden,
                g.mul(repeat_ax(g.reshape(mod_img_all[2], g.constant({3}, sh3_gate, DType::INT32)), img_seq_len, 1),
                      img_attn_out));
            txt_hidden =
                g.add(txt_hidden, g.mul(repeat_ax(g.reshape(mod_txt_all[2], g.constant({3}, sh3_gate, DType::INT32)),
                                                  cfg.text_max_seq, 1),
                                        txt_attn_out));
            ...
            auto ffn = [&](uint32_t h, uint32_t shift, uint32_t scale, uint32_t gate, const std::string &pfx, int seq) {
                ...
                int32_t step[] = {1, 1, 1};
                uint32_t ff_gate = g.slice(ff, g.constant({3}, s_gate, DType::INT32),
                                           g.constant({3}, e_gate, DType::INT32), g.constant({3}, step, DType::INT32));
                int32_t s_up[] = {0, 0, (int32_t)cfg.mlp_hidden};
                int32_t e_up[] = {1, (int32_t)seq, (int32_t)cfg.mlp_hidden * 2};
                uint32_t ff_up = g.slice(ff, g.constant({3}, s_up, DType::INT32), g.constant({3}, e_up, DType::INT32),
                                         g.constant({3}, step, DType::INT32));

                uint32_t out = linear(g.mul(silu_atomic(ff_gate, 1, seq, cfg.mlp_hidden), ff_up), pfx + "_out.weight",
                                      cfg.mlp_hidden, cfg.hidden_size);
                return g.add(h,
                             g.mul(repeat_ax(g.reshape(gate, g.constant({3}, sh3_gate, DType::INT32)), seq, 1), out));
            };

            img_hidden = ffn(img_hidden, mod_img_all[3], mod_img_all[4], mod_img_all[5], p + ".ff.linear", img_seq_len);
            txt_hidden = ffn(txt_hidden, mod_txt_all[3], mod_txt_all[4], mod_txt_all[5], p + ".ff_context.linear",
                             cfg.text_max_seq);
        }
        ...
        for (uint32_t i = 0; i < cfg.num_single_layers; ++i)
        {
            ...
            uint32_t mod = apply_mod(layer_norm_atomic(combined, total_seq_len), mod_single_all[0], mod_single_all[1],
                                     total_seq_len);
            uint32_t fused = linear(mod, p + ".attn.to_qkv_mlp_proj.weight", cfg.hidden_size,
                                    cfg.hidden_size * 3 + cfg.mlp_hidden * 2);

            auto slice_feat = [&](int start, int end) {
                int32_t s[] = {0, 0, start};
                int32_t e[] = {1, (int32_t)total_seq_len, end};
                int32_t step[] = {1, 1, 1};
                return g.slice(fused, g.constant({3}, s, DType::INT32), g.constant({3}, e, DType::INT32),
                               g.constant({3}, step, DType::INT32));
            };
            ...
            auto reshape_for_attn = [&](uint32_t x) {
                ...
                int32_t p_[] = {0, 2, 1, 3};
                return g.contiguous(g.permute(g.reshape(g.contiguous(x), g.constant({4}, sh4, DType::INT32)),
                                              g.constant({4}, p_, DType::INT32)));
            };

            uint32_t q = apply_rope_2d_consecutive(rms_norm_atomic(reshape_for_attn(slice_feat(0, cfg.hidden_size)),
                                                                   p + ".attn.norm_q.weight", total_seq_len,
                                                                   cfg.num_heads, cfg.head_dim),
                                                   rope_cos, rope_sin, total_seq_len);
            uint32_t k = apply_rope_2d_consecutive(
                rms_norm_atomic(reshape_for_attn(slice_feat(cfg.hidden_size, cfg.hidden_size * 2)),
                                p + ".attn.norm_k.weight", total_seq_len, cfg.num_heads, cfg.head_dim),
                rope_cos, rope_sin, total_seq_len);
            ...
            uint32_t mlp_up =
                slice_feat(cfg.hidden_size * 3 + cfg.mlp_hidden, cfg.hidden_size * 3 + cfg.mlp_hidden * 2);

            int32_t p_k[] = {0, 1, 3, 2};
            uint32_t scores = g.mul(g.dot(q, g.contiguous(g.permute(k, g.constant({4}, p_k, DType::INT32)))),
                                    expand_scalar_to_4d(1.0f / std::sqrt((float)cfg.head_dim), 1, cfg.num_heads,
                                                        total_seq_len, total_seq_len));
            int32_t ax_ = -1;
            uint32_t shifted =
                g.add(scores, g.neg(repeat_ax(g.max(scores, g.constant({1}, &ax_, DType::INT32)), total_seq_len, 3)));
            uint32_t exps =
                g.pow(expand_scalar_to_4d(2.7182818f, 1, cfg.num_heads, total_seq_len, total_seq_len), shifted);
            uint32_t probs = g.div(exps, repeat_ax(g.sum(exps, g.constant({1}, &ax_, DType::INT32)), total_seq_len, 3));

            uint32_t ctx = g.dot(probs, v);
            int32_t p_c[] = {0, 2, 1, 3};
            ctx = g.reshape(g.contiguous(g.permute(ctx, g.constant({4}, p_c, DType::INT32))),
                            g.constant({3},
                                       std::vector<int32_t>{1, (int32_t)total_seq_len, (int32_t)cfg.hidden_size}.data(),
                                       DType::INT32));

            uint32_t mlp_out = g.mul(silu_atomic(mlp_gate, 1, total_seq_len, cfg.mlp_hidden), mlp_up);
            int32_t ax_concat = 2;
            uint32_t out = linear(g.concat({ctx, mlp_out}, g.constant({1}, &ax_concat, DType::INT32)),
                                  p + ".attn.to_out.weight", cfg.hidden_size + cfg.mlp_hidden, cfg.hidden_size);

            int32_t sh3_gate[] = {1, 1, (int32_t)cfg.hidden_size};
            combined =
                g.add(combined, g.mul(repeat_ax(g.reshape(mod_single_all[2], g.constant({3}, sh3_gate, DType::INT32)),
                                                total_seq_len, 1),
                                      out));
        }
        ...
        uint32_t img_out =
            g.slice(combined, g.constant({3}, slice_s, DType::INT32), g.constant({3}, slice_e, DType::INT32),
                    g.constant({3}, slice_step_out, DType::INT32));

        auto final_mods = compute_mods(t_emb_silu, "norm_out.linear.weight", 2);
        uint32_t mod_out =
            apply_mod(layer_norm_atomic(img_out, img_seq_len), final_mods[1], final_mods[0], img_seq_len);
        ...
        return g.contiguous(
            g.permute(g.reshape(output, g.constant({4}, sh_out, DType::INT32)), g.constant({4}, p_out, DType::INT32)));
    }
};