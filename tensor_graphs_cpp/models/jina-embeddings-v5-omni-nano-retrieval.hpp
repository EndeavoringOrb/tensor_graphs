...
class JinaV5OmniNanoRetrievalModel
{
  private:
    ...
    LogicalId expand_scalar_to_2d(float val, uint32_t dim0, uint32_t dim1)
    {
        LogicalId node = g.constant({1}, &val, DType::FLOAT32);
        int32_t shape_2d[] = {1, 1};
        LogicalId out = g.reshape(node, g.constant({2}, shape_2d, DType::INT32));
        if (dim0 > 1)
    ...
    LogicalId expand_scalar_to_3d(float val, uint32_t dim0, uint32_t dim1, uint32_t dim2)
    {
        ...
        LogicalId out = g.reshape(node, g.constant({3}, shape_3d, DType::INT32));
        if (dim0 > 1)
            out = repeat_3d_axis(out, dim0, 0);
        if (dim1 > 1)
            out = repeat_3d_axis(out, dim1, 1);
    ...
    LogicalId expand_1d_to_3d(LogicalId vec_id, uint32_t vec_len, uint32_t dim0, uint32_t dim1)
    {
        ...
        LogicalId out = g.reshape(vec_id, g.constant({3}, shape_3d, DType::INT32));
        if (dim0 > 1)
            out = repeat_3d_axis(out, dim0, 0);
        if (dim1 > 1)
            out = repeat_3d_axis(out, dim1, 1);
                         ...
                         float eps = 1e-6f)
    {
        ...
        LogicalId sum_x = g.sum(x, axis_node);
        LogicalId d_node = expand_scalar_to_3d((float)D, 1, S, 1);
        LogicalId mean_val = g.div(sum_x, d_node);
        LogicalId mean = repeat_3d_axis(mean_val, D, 2);

        LogicalId x_sub = g.add(x, g.neg(mean));
        LogicalId sq = g.mul(x_sub, x_sub);
        LogicalId sum_sq = g.sum(sq, axis_node);
        LogicalId var = g.div(sum_sq, d_node);

        LogicalId eps_node = expand_scalar_to_3d(eps, 1, S, 1);
        LogicalId var_plus_eps = g.add(var, eps_node);

        LogicalId sqrt_exp = expand_scalar_to_3d(0.5f, 1, S, 1);
        LogicalId std_dev = g.pow(var_plus_eps, sqrt_exp);

        LogicalId one_node = expand_scalar_to_3d(1.0f, 1, S, 1);
        LogicalId inv_std = g.div(one_node, std_dev);
        LogicalId inv_std_expanded = repeat_3d_axis(inv_std, D, 2);

        LogicalId normalized = g.mul(x_sub, inv_std_expanded);

        if (!w_name.empty())
        {
            LogicalId w = weight(w_name);
            LogicalId w_exp = expand_1d_to_3d(w, D, 1, S);
            normalized = g.mul(normalized, w_exp);
        }
        if (!b_name.empty())
        {
            LogicalId b = weight(b_name);
            LogicalId b_exp = expand_1d_to_3d(b, D, 1, S);
    ...
    LogicalId rms_norm(LogicalId x_id, const std::string &w_name, uint32_t S, uint32_t D, float eps)
    {
        ...
        LogicalId n_node = expand_scalar_to_3d((float)D, 1, S, 1);
        LogicalId mean_sq = g.div(sum_sq, n_node);
        LogicalId eps_expanded = expand_scalar_to_3d(eps, 1, S, 1);
        LogicalId mean_sq_plus_eps = g.add(mean_sq, eps_expanded);
        LogicalId sqrt_node = expand_scalar_to_3d(0.5f, 1, S, 1);
                     ...
                     uint32_t S)
    {
        LogicalId w = weight(w_name);
        int32_t p[] = {1, 0};
        LogicalId w_t = g.permute(w, g.constant({2}, p, DType::INT32));
        w_t = g.contiguous(w_t);
        int32_t sh3[] = {1, (int32_t)in_d, (int32_t)out_d};
        LogicalId out = g.dot(x, g.reshape(w_t, g.constant({3}, sh3, DType::INT32)));
        if (!b_name.empty())
        {
            LogicalId b = weight(b_name);
            LogicalId b_exp = expand_1d_to_3d(b, out_d, 1, S);
            out = g.add(out, b_exp);
        }
        return out;
    }
    ...
    {
        LogicalId neg_one = expand_scalar_to_3d(-1.0f, N, L, D);
        LogicalId neg_x = g.mul(x_id, neg_one);
        LogicalId e_node = expand_scalar_to_3d(2.718281828459045f, N, L, D);
        LogicalId exp_neg_x = g.pow(e_node, neg_x);
        LogicalId one_node = expand_scalar_to_3d(1.0f, N, L, D);
    ...
    LogicalId gelu_exact(LogicalId x_id, uint32_t S, uint32_t D)
    {
        ...
        LogicalId a1 = expand_scalar_to_3d(0.254829592f, 1, S, D);
        LogicalId a2 = expand_scalar_to_3d(-0.284496736f, 1, S, D);
        LogicalId a3 = expand_scalar_to_3d(1.421413741f, 1, S, D);
        LogicalId a4 = expand_scalar_to_3d(-1.453152027f, 1, S, D);
        LogicalId a5 = expand_scalar_to_3d(1.061405429f, 1, S, D);
        ...
        // exp(-x_scaled^2)
        LogicalId e_node = expand_scalar_to_3d(2.718281828459045f, 1, S, D);
    ...
    LogicalId softmax_4d(LogicalId scores, uint32_t S, uint32_t num_heads)
    {
        ...
        LogicalId max_expanded = repeat_4d_axis(max_s, S, 3);
        LogicalId shifted = g.add(scores, g.neg(max_expanded));

        LogicalId e_node = expand_scalar_to_4d(2.718281828459045f, 1, num_heads, S, S);
        LogicalId exps = g.pow(e_node, shifted);
        LogicalId sums = g.sum(exps, axis_node);
        LogicalId sums_expanded = repeat_4d_axis(sums, S, 3);
    ...
    LogicalId patch_embed(LogicalId x, uint32_t num_patches)
    {
        LogicalId w = weight("vision_tower.patch_embed.proj.weight"); // (768, 3, 2, 16, 16)
        int32_t sh2[] = {768, 1536};
        LogicalId w_2d = g.reshape(w, g.constant({2}, sh2, DType::INT32)); // (768, 1536)
        int32_t p[] = {1, 0};
        LogicalId w_t = g.contiguous(g.permute(w_2d, g.constant({2}, p, DType::INT32))); // (1536, 768)
        int32_t sh3[] = {1, 1536, 768};
        LogicalId w_3d = g.reshape(w_t, g.constant({3}, sh3, DType::INT32));
        // (1, num_patches, 1536) × (1, 1536, 768) → (1, num_patches, 768)
        LogicalId out = g.dot(x, w_3d);

        // [FIX] Add bias
        LogicalId b = weight("vision_tower.patch_embed.proj.bias"); // (768,)
        LogicalId b_exp = expand_1d_to_3d(b, 768, 1, num_patches);
    ...
    LogicalId compute_pos_embed(uint32_t num_patches)
    {
        ...
        LogicalId idx00_node = g.constant({num_patches}, idx00.data(), DType::INT32);
        LogicalId idx01_node = g.constant({num_patches}, idx01.data(), DType::INT32);
        LogicalId idx10_node = g.constant({num_patches}, idx10.data(), DType::INT32);
        LogicalId idx11_node = g.constant({num_patches}, idx11.data(), DType::INT32);

        // Load pos_embed table: (2304, 768)
        LogicalId pos_table = weight("vision_tower.pos_embed.weight");
        ...
        LogicalId sh3_node = g.constant({3}, sh3_np, DType::INT32);
        ...
        LogicalId w01_3d = make_weight_3d(w01);
        LogicalId w10_3d = make_weight_3d(w10);
        LogicalId w11_3d = make_weight_3d(w11);

        // Weighted sum: pos = w00*g00 + w01*g01 + w10*g10 + w11*g11
        LogicalId pos = g.mul(g00, w00_3d);
    ...
    LogicalId vision_attention(LogicalId x, int layer_idx, LogicalId cos, LogicalId sin, uint32_t S)
    {
        ...
        int32_t p_k[] = {0, 1, 3, 2};
        LogicalId k_t = g.contiguous(g.permute(k, g.constant({4}, p_k, DType::INT32)));
        LogicalId scores = g.dot(q, k_t); // (1, num_heads, S, S)

        LogicalId probs = softmax_4d(scores, S, cfg.vision_num_heads);
        LogicalId attn_out = g.dot(probs, v); // (1, num_heads, S, head_dim)

        int32_t p_ctx[] = {0, 2, 1, 3};
        LogicalId ctx_perm = g.contiguous(g.permute(attn_out, g.constant({4}, p_ctx, DType::INT32)));
    ...
    LogicalId vit_encoder(LogicalId patch_input)
    {
        uint32_t S = cfg.num_patches; // 1024

        LogicalId h = patch_embed(patch_input, S); // (1, 1024, 768)
    ...
    LogicalId text_attention(LogicalId x, int layer_idx, LogicalId cos, LogicalId sin, uint32_t S)
    {
        ...
        LogicalId probs = softmax_4d(scores, S, cfg.text_num_heads);
    ...
    LogicalId l2_normalize(LogicalId x, uint32_t D)
    {
        ...
        LogicalId axis_node = g.constant({1}, &axis_val, DType::INT32);
        LogicalId sum_sq = g.sum(x_sq, axis_node);
        LogicalId eps_expanded = expand_scalar_to_2d(1e-12f, 1, 1);
        LogicalId sum_sq_plus_eps = g.add(sum_sq, eps_expanded);
        LogicalId std = g.pow(sum_sq_plus_eps, expand_scalar_to_2d(0.5f, 1, 1));
        LogicalId inv_std = g.div(expand_scalar_to_2d(1.0f, 1, 1), std);
    ...
    LogicalId build_chat_template_embeds()
    {
        ...
        LogicalId embed_table = weight("language_model.embed_tokens.weight");
  ...
  public:
    ...
    LogicalId build_graph(LogicalId patch_input)
    {
        ...
        LogicalId chat_embeds = build_chat_template_embeds(); // (1, 14, 768)
