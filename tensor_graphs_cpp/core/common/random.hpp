...
{
    // Generate two independent random numbers from a uniform distribution in the
    // range (0,1)
    ...
    // Box-Muller transform to convert uniform random numbers to normal
    // distribution
...
/**
 * Generates a random integer value in the range [low, high] (including both
 * endpoints)
 *
...
{
    return std::min(static_cast<int>(randFloat(seed) * static_cast<float>(high - low + 1) + static_cast<float>(low)),
                    high);
}

/**
 * Generates a random unsigned integer value in the range [0, high] (including
 * both endpoints)
 *
 * @param seed A 32-bit unsigned integer used as the seed.
...
{
    return std::min(static_cast<uint32_t>(randFloat(seed) * static_cast<float>(high + 1)), high);
}
} // namespace tg_random