...
struct BatchFlattenDot : public Rule
{
    std::unordered_set<uint32_t> visited_enodes;

    std::string name() const override { return "BatchFlattenDot"; }
    ...
    bool match(const EGraph &egraph, uint32_t eNodeIdx, const std::unordered_set<uint32_t> &protectedEClasses) override
    {
        ...
        // Apply only to ranks != 3 to avoid infinite loops and redundancies on native 3D dots
        if (enode.shape.size() == 3)
            return false;
    ...
    void apply(EGraph &egraph, uint32_t eNodeIdx, const std::unordered_set<uint32_t> &protectedEClasses, std::unordered_map<uint32_t, uint32_t> &eclassToLogical) override
    {
        ...
        uint32_t M = aEClass.shape[rankA - 2];
        uint32_t K = aEClass.shape[rankA - 1];
        uint32_t N = bEClass.shape[rankB - 1];
        ...
        // 1. Contiguous -> Reshape A to [B, M, K]
        // Explicitly ensuring CONTIGUOUS allows the reshape to inherit mathematically valid contiguous strides
        uint32_t aContig = addOpToEGraph(egraph, OpType::CONTIGUOUS, {aClass}, aEClass.shape, calcContiguousStrides(aEClass.shape), 0, aEClass.dtype, aEClass.backend);
        std::vector<int32_t> aShape3D = {(int32_t)B, (int32_t)M, (int32_t)K};
        uint32_t aShape3DId = egraph.getOrAddConstantData<int32_t>({3}, DType::INT32, Backend::CPU, aShape3D);
        std::vector<uint32_t> aShape3DU32 = {B, M, K};
        uint32_t aReshape = addOpToEGraph(egraph, OpType::RESHAPE, {aContig, aShape3DId}, aShape3DU32, calcContiguousStrides(aShape3DU32), 0, aEClass.dtype, aEClass.backend);

        // 2. Contiguous -> Reshape B to [B, K, N]
        uint32_t bContig = addOpToEGraph(egraph, OpType::CONTIGUOUS, {bClass}, bEClass.shape, calcContiguousStrides(bEClass.shape), 0, bEClass.dtype, bEClass.backend);
        std::vector<int32_t> bShape3D = {(int32_t)B, (int32_t)K, (int32_t)N};
        uint32_t bShape3DId = egraph.getOrAddConstantData<int32_t>({3}, DType::INT32, Backend::CPU, bShape3D);
        std::vector<uint32_t> bShape3DU32 = {B, K, N};
        uint32_t bReshape = addOpToEGraph(egraph, OpType::RESHAPE, {bContig, bShape3DId}, bShape3DU32, calcContiguousStrides(bShape3DU32), 0, bEClass.dtype, bEClass.backend);

        // 3. Dot 3D
        std::vector<uint32_t> dotShape = {B, M, N};
        uint32_t dot3D = addOpToEGraph(egraph, OpType::DOT, {aReshape, bReshape}, dotShape, calcContiguousStrides(dotShape), 0, enode.dtype, enode.backend);

        // 4. Reshape back to original shape
        std::vector<int32_t> outShapeOri;
        for (uint32_t d : enode.shape)
        {
            outShapeOri.push_back((int32_t)d);
        }
        uint32_t outShapeOriId = egraph.getOrAddConstantData<int32_t>({(uint32_t)outShapeOri.size()}, DType::INT32, Backend::CPU, outShapeOri);
        uint32_t outReshape = addOpToEGraph(egraph, OpType::RESHAPE, {dot3D, outShapeOriId}, enode.shape, calcContiguousStrides(enode.shape), 0, enode.dtype, enode.backend);

        // Merge the final reshape back into the original DOT eclass
...
struct CastPushDown : public Rule
{
    std::unordered_set<uint32_t> visited_enodes;

    std::string name() const override { return "CastPushDown"; }
    ...
    bool match(const EGraph &egraph, uint32_t eNodeIdx, const std::unordered_set<uint32_t> &protectedEClasses) override
    {
        ...
        const ENode &enode = egraph.getENodes()[eNodeIdx];
        if (enode.opType == OpType::PERMUTE || enode.opType == OpType::CONTIGUOUS || enode.opType == OpType::RESHAPE || enode.opType == OpType::SLICE)
        {
            uint32_t childClass = egraph.findConst(enode.children[0]);
            for (uint32_t childNodeIdx : egraph.getEClass(childClass).enodes)
            {
                if (egraph.getENodes()[childNodeIdx].opType == OpType::CAST)
                    return true;
            }
        }
        return false;
    ...
    void apply(EGraph &egraph, uint32_t eNodeIdx, const std::unordered_set<uint32_t> &protectedEClasses, std::unordered_map<uint32_t, uint32_t> &eclassToLogical) override
    {
        ...
        for (uint32_t childNodeIdx : egraph.getEClass(childClass).enodes)
        {
            const ENode &castNode = egraph.getENodes()[childNodeIdx];
            if (castNode.opType == OpType::CAST)
            {
                uint32_t grandChildClass = egraph.findConst(castNode.children[0]);
                const EClass &srcClass = egraph.getEClass(grandChildClass);

                std::vector<uint32_t> newChildren = enode.children;
                newChildren[0] = grandChildClass;

                std::vector<uint64_t> newStrides;
                uint64_t newViewOffset = 0;

                if (enode.opType == OpType::CONTIGUOUS)
                {
                    newStrides = calcContiguousStrides(enode.shape);
                }
                else if (enode.opType == OpType::PERMUTE)
                {
                    auto dims = getConstInt32(egraph, enode.children[1]);
                    newStrides.resize(dims.size());
                    for (size_t i = 0; i < dims.size(); ++i)
                        newStrides[i] = srcClass.strides[dims[i]];
                    newViewOffset = srcClass.viewOffset;
                }
                else if (enode.opType == OpType::RESHAPE)
                {
                    newStrides = calcContiguousStrides(enode.shape);
                    newViewOffset = srcClass.viewOffset;
                }
                else if (enode.opType == OpType::SLICE)
                {
                    auto starts = getConstInt32(egraph, enode.children[1]);
                    auto steps = getConstInt32(egraph, enode.children[3]);
                    newStrides = srcClass.strides;
                    newViewOffset = srcClass.viewOffset;
                    for (size_t d = 0; d < starts.size(); ++d)
                    {
                        int32_t start = starts[d];
                        if (start < 0)
                            start += srcClass.shape[d];
                        newViewOffset += start * newStrides[d];
                        newStrides[d] *= steps[d];
                    }
                }

                uint32_t newOpClass = addOpToEGraph(egraph, enode.opType, newChildren, enode.shape, newStrides, newViewOffset, srcClass.dtype, enode.backend);
                addOpToEGraph(egraph, OpType::CAST, {newOpClass}, enode.shape, calcContiguousStrides(enode.shape), 0, enode.dtype, enode.backend, eclassId);
            }
        }
    }
};