...
class MallocEnv
{
  public:
    ...
    MallocEnv(uint64_t cap, const std::vector<ParallelBuffer> &bufs) : mem_cap(cap), buffers(bufs)
    {
        N = buffers.size();
        adj.resize(N);
        for (int i = 0; i < N; ++i)
    ...
    std::vector<bool> get_valid_actions() const
    {
        ...
        for (int i = 0; i < N; ++i)
        {
            ...
            if (offset_i < global_offset_max)
                continue;
            if (offset_i == last_chosen_offset && buffers[i].id < last_chosen_id)
                continue;
            if (offset_i >= h_min)
                continue;
            if (mem_cap != std::numeric_limits<uint64_t>::max() &&
                static_cast<uint64_t>(offset_i) + buffers[i].size > mem_cap)
                continue;
...
{
    py::class_<ParallelBuffer>(m, "ParallelBuffer").def(py::init<uint32_t, int64_t, float, float>());
