                ...
                try:
                    ...
                    if config["source"] == "record":
                        ...
                        if not kernel_result:
                            concrete_sigs_int = [
                                TensorSignature(DType.INT32, tuple(s), backend)
                        ...
                        for sig in concrete_sigs:
                            inputs.append(
                                DataGenerator.random_tensor(sig.shape, sig.dtype)
                            )
                        ...
                        for i, inp in enumerate(real_inputs):
                            ...
                            if hasattr(inp, "dtype"):
                                ...
                                if "int" in dt_s:
                                    ...
                                    dt = DType.BOOL
                            dummy_parents.append(
                                TensorNode(OpType.INPUT, dt, [], shape=s, name=f"p{i}")
                            )
