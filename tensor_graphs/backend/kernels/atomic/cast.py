...
def cast_fp8_to_fp32(inputs, attrs=None):
    val = inputs[0]
    return val.astype(np.float32)


@KernelRegistry.register_cast(src=DType.INT32, dst=DType.FP32)
def cast_int32_to_fp32(inputs, attrs=None):
    val = inputs[0]
    return val.astype(np.float32)
...
def cast_generic_int32(inputs, attrs=None):
...
def cast_generic_fp32(inputs, attrs=None):
